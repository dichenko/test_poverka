...
const envSchema = z.object({
  ...
  OCR_API_URL: z.string().url().default("https://ocr.poverka-bot.ru"),
  OCR_API_KEY: z.preprocess((value) => value === "" ? undefined : value, z.string().min(1).optional()),
  OCR_HTTP_TIMEOUT_MS: z.coerce.number().int().positive().default(120000),
  OCR_MAX_RETRIES: z.coerce.number().int().min(0).max(10).default(2),
  OCR_WORKER_POLL_INTERVAL_MS: z.coerce.number().int().positive().default(5000),
  OCR_RETRY_DELAY_MS: z.coerce.number().int().positive().default(5000),
